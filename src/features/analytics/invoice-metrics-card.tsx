...
export function InvoiceMetricsCard({ metrics }: InvoiceMetricsCardProps) {
  const stats = [
    ...
    { label: "Pending", value: metrics.pending_invoices, color: "text-amber-600" },
    { label: "Awaiting", value: metrics.awaiting_confirmation, color: "text-blue-600" },
  ...
  return (
    <div className="rounded-lg border border-brand-border bg-white p-4 sm:p-6 shadow-card">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base sm:text-lg font-semibold text-brand-text">Invoice Metrics</h3>
      ...
      <div className="space-y-2">
        {stats.map((stat) => (
          <div key={stat.label} className="flex items-center justify-between py-2 border-b border-brand-border last:border-0">
