  ...
  return (
    <div className="space-y-4 sm:space-y-6 rounded-lg border border-brand-border bg-white p-4 sm:p-6 shadow-card">
      ...
      <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4 border-b border-brand-border pb-4">
        <div className="flex-1 min-w-0">
          <h2 className="text-lg sm:text-xl font-bold text-brand-text truncate">
            {invoice.invoice_type === "expense" ? "Expense" : "Invoice"}{" "}
            {invoice.invoice_id}
          </h2>
          <p className="mt-1 text-xs sm:text-sm text-brand-textMuted">
            Created {formatIsoDate(invoice.created_at ?? null)}
          </p>
          {invoice.paid_at && (
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span
            className={`inline-flex items-center rounded-full px-3 py-1.5 text-xs font-semibold uppercase tracking-wide ${statusToneClass(
              statusMeta.tone
            )}`}
            aria-live="polite"
      ...
      <section className="space-y-3 sm:space-y-5">
        ...
        {invoice.invoice_type === "expense" && (
          <>
            ...
            {invoice.receipt_url && (
              <div className="rounded-lg border border-brand-border bg-brand-background p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-brand-textMuted mb-3">
        ...
        {invoice.invoice_type === "expense" ? (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 sm:p-4">
            <p className="text-xs sm:text-sm font-medium text-emerald-900">
              ✅ This expense was automatically marked as paid when recorded for
          ...
          invoice.status === "awaiting_confirmation" && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
              <p className="text-sm font-medium text-amber-900">
                ⚠️ Customer reported that they have transferred the funds.
                Please check your bank and move the status to &ldquo;Paid&rdquo;
                once cleared to automatically send their receipt.
              </p>
            </div>
          )}
        ...
        {invoice.invoice_type !== "expense" && shareLink && (
          <div className="rounded-lg border border-brand-border bg-brand-background p-4">
            ...
            <p className="mt-1 text-xs text-brand-textMuted">
              Share this link so your customer can view payment instructions and
              confirm their transfer.
            </p>
      ...
      <section className="border-t border-brand-border pt-4 sm:pt-5">
        <h3 className="mb-3 sm:mb-4 text-sm sm:text-base font-bold text-brand-text">
          Line Items
        </h3>
                ...
                <table className="min-w-full divide-y divide-brand-border text-xs sm:text-sm">
                  ...
                  <tbody className="divide-y divide-brand-border bg-white">
                    {invoice.lines.map((line) => (
                      <tr key={line.id}>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 font-medium text-brand-text break-words max-w-[200px] sm:max-w-none">
                          {line.description}
                        </td>
